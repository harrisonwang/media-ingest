...
func appStateDir() (string, error) {
	...
	if runtime.GOOS == "windows" {
		if v := strings.TrimSpace(os.Getenv("LOCALAPPDATA")); v != "" {
			return filepath.Join(v, "youtube"), nil
		}
	}
	...
	return filepath.Join(base, "youtube"), nil
}
...
func findChromeExecutable() (string, error) {
	if p := strings.TrimSpace(os.Getenv("YOUTUBE_CHROME_PATH")); p != "" {
		...
		return "", fmt.Errorf("YOUTUBE_CHROME_PATH 无效: %s", p)
	}
	...
	return "", errors.New("未找到 Chrome。可通过 YOUTUBE_CHROME_PATH 指定 chrome 可执行文件路径")
}
...
func exportCookiesFromChromeCDP(chromePath, profileDir string, headless bool) (string, func(), error) {
	...
	f, err := os.CreateTemp("", "youtube-cookies-*.txt")
	if err != nil {
		return "", nil, err
	}
	path := f.Name()
...
func writeNetscapeCookieFile(path string, cookies []chromeCookie) error {
	...
	_, _ = fmt.Fprintln(f, "# This file was generated by youtube. DO NOT EDIT.")
	...
	for _, c := range cookies {
		...
		_, _ = fmt.Fprintf(f, "%s\t%s\t%s\t%s\t%d\t%s\t%s\n",
			domain, includeSubdomains, c.Path, secure, exp, c.Name, c.Value)
	}
